		...
		<div className='Options'>
			...
			<Grid container className='optionContainerXS' pb={5} sx={{ borderBottom: 1, borderColor: '#cbd5e1' }} >
				<Grid item xs={12} sm={12} md={12} lg={12} className='qualityContainer'>
					...
					<div style={{ marginTop: -25, float: 'right', display: 'flex', color: data.options.lossless ? '#b7babe' : '#1f2937' }}>
						{data.options.quality}
					</div>
					<div>{data.options.lossless ?
						<Sliderr value={data.options.quality} disabled /> :
						<Sliderr value={data.options.quality} min={1} max={100} onChange={(e, nv) => dispatch({ type: 'change-quality', payload: nv })} />}
					</div>
				</Grid>
				<Grid item style={{ color: '#1f2937' }} xs={12} sm={12} md={12} lg={12}>
					<Grid container direction='row'>
						<Grid item xs={8}>Lossless</Grid>
						<Grid style={{ marginTop: -3, marginBottom: 9, textAlign: 'end' }} item xs={4}>
							<Switchh value={data.options.lossless} onChange={(e) => dispatch({ type: 'toggle-lossless', payload: e.target.checked })} />
						</Grid>
					</Grid>
					<Grid container direction='row'>
						<Grid item xs={8}>Keep Metadata</Grid>
						<Grid style={{ marginTop: 0, textAlign: 'end' }} item xs={4}>
							<Switchh value={data.options.keepMetadata} onChange={(e) => dispatch({ type: 'toggle-keepmetadata', payload: e.target.checked })} />
						</Grid>
					</Grid>
				</Grid>
			</Grid>
			...
			<Grid container className='optionContainerSM' pb={5} sx={{ borderBottom: 1, borderColor: '#cbd5e1' }} >
				...
				<Grid item xs={12} sm={12} md={12} lg={12} style={{ color: '#1f2937' }}>
					<Grid container direction='row'>
						<Grid item xs={8}>Lossless Compression</Grid>
						<Grid style={{ marginTop: -3, marginBottom: 9, textAlign: 'end' }} item xs={4}>
							<Switchh value={data.options.lossless} onChange={(e) => dispatch({ type: 'toggle-lossless', payload: e.target.checked })} />
						</Grid>
					</Grid>
			...
			<Grid container className='optionContainerLG' pb={5} sx={{ borderBottom: 1, borderColor: '#cbd5e1' }}>
				<Grid item xs={6} sm={6} md={6} lg={6} className='qualityContainer' pr={4}>
					...
					<div>{data.options.lossless ?
						<Sliderr value={data.options.quality} disabled /> :
						<Sliderr value={data.options.quality} min={1} max={100} onChange={(e, nv) => { console.log(2); dispatch({ type: 'change-quality', payload: nv }); }} />}
					</div>
				</Grid>
